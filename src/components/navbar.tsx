...
export default function Navbar() {
  ...
  const options = [
    ...
    { label: 'He got engaged', link: '/He_got_engaged' },
  ...
  return (
    <nav className='flex justify-center items-center bg-white py-5'>
      <div className='flex items-center space-x-3 lg:space-x-7 ring-1 ring-primary-400 p-3 rounded-md'>
        ...
        <Link className={router.pathname == "/articles" ? "active" : ""} href="./articles">
          <div className='flex items-center space-x-1 lg:space-x-2 text-black  hover:bg-gray-100 hover:p-2 hover:rounded-md hover:transition-all ease-in-out duration-300'>
            <span className='text-xs lg:text-sm'>مقالات-الشيخ</span>
            <SlBookOpen />
          </div>
        </Link>
        <div>
        ...
        <Link className={router.pathname == "/" ? "active" : ""} href="./">
          <div className={`flex items-center space-x-1 lg:space-x-2 text-black`}>
